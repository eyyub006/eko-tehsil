...
# Ensure DB is created even in production (Gunicorn)
with app.app_context():
    # Create tables if not strictly existing (using a simplified check or just try-except)
    # Ideally we use migrations, but for this project:
    db.create_all()
    # Check if admin exists to avoid re-seeding every restart if persistency works
